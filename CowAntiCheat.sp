...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_cowac_sourcebans", checkSB);
}

public Action checkSB(int client, int args)
{
	if(sourcebans)
		PrintToChat(client, "[\x02CowAC\x01] Sourcebans \x04is connected\x01!");
	else
		PrintToChat(client, "[\x02CowAC\x01] Sourcebans is \x02not connected\x01!");
...
{
	sourcebans = LibraryExists("sourcebanspp");
}

public void OnLibraryRemoved(const char[] name)
{
	if(StrEqual(name, "sourcebanspp"))
		sourcebans = false;
}
